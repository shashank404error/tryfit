...
##RETR_TREE
def redesign_image(val,overlay):
    threshold = val
...
######################end of param####################################
    ...
    if(img_src=="samyak"):
     ...
     cv.moveWindow("shirt1", l,600-Croped.shape[0])
    ##cv2.imwrite("final.png",im1)
    cv.imshow("im_rec",im2)
...
#load image
img_src_jpeg=img_src+".jpeg"
output_img=img_src+".png"

response = requests.post(
    'https://api.remove.bg/v1.0/removebg',
    files={'image_file': open(img_src_jpeg, 'rb')},
    data={'size': 'auto'},
    headers={'X-Api-Key': '1zy7EQYXjruveonVn7vmxqZm'},
)
